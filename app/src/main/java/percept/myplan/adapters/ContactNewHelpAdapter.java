...
public class ContactNewHelpAdapter extends BaseAdapter implements
        StickyListHeadersAdapter, SectionIndexer, Filterable {

    private final Context mContext;
    private List<ContactDisplay> LIST_CONTACT;
    private List<ContactDisplay> LIST_FIX_CONTACT;
    ...
    public ContactNewHelpAdapter(Context context, List<ContactDisplay> lstContact, boolean SINGLE_CHECK, int helpCount, boolean isFromHelp) {
        mContext = context;
    ...
    private int[] getSectionIndices() {
        if (LIST_CONTACT.size() > 0) {
            ...
            if (!TextUtils.isEmpty(LIST_CONTACT.get(0).getFirst_name()))
                lastFirstChar = LIST_CONTACT.get(0).getFirst_name().charAt(0);
            else lastFirstChar = '#';
            ...
            for (int i = 1; i < LIST_CONTACT.size(); i++) {
                ...
                } else if (LIST_CONTACT.get(i).getFirst_name().charAt(0) != lastFirstChar) {
    ...
    private Character[] getSectionLetters() {
        ...
        for (int i = 0; i < mSectionIndices.length; i++) {
            if (TextUtils.isEmpty(LIST_CONTACT.get(mSectionIndices[i]).getFirst_name()))
                letters[i] = '#';
            else
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (TextUtils.isEmpty(LIST_CONTACT.get(position).getFirst_name()))
            ...
            holder.TV_CONTACTNAME.setText(LIST_CONTACT.get(position).getFirst_name() + " " + LIST_CONTACT.get(position).getLast_name());
    ...
    public View getHeaderView(int position, View convertView, ViewGroup parent) {
        ...
        if (TextUtils.isEmpty(LIST_CONTACT.get(position).getFirst_name()))
            ...
            headerChar = LIST_CONTACT.get(position).getFirst_name().subSequence(0, 1);
    ...
    private class LocFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint == null || constraint.length() == 0) {
                ...
                List<ContactDisplay> nPlanetList = new ArrayList<ContactDisplay>();

                for (ContactDisplay p : LIST_FIX_CONTACT) {
                    if (p.getFirst_name().toString().trim().toUpperCase()
        ...
        protected void publishResults(CharSequence charSequence,
            ...
            LIST_CONTACT = (List<ContactDisplay>) results.values;
            notifyDataSetChanged();
